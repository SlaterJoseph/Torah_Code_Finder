...
def concat_file(file):
    ...
    new_file = open('processed_texts/' + os.path.basename(file),'a', encoding='utf-8')  # created a file for the new processed text, places in processde_texts
...
def process_code(nums, file):
    sequence = nums.split(',')
    my_file = open(file, encoding='utf-8')
    parsed_string = ''
    for line in my_file:
        print(len(line))# Loops through the lines
